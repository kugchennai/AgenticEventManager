...
import { ClipboardCheck, Plus, Pencil, Trash2, X, ChevronDown, ChevronRight, Copy } from "lucide-react";
...
function TemplateFormModal({
  ...
  const initialTasks: Record<Section, DefaultTask[]> = { PRE_EVENT: [], ON_DAY: [], POST_EVENT: [] };
  if (template?.defaultTasks?.length) {
    for (const t of template.defaultTasks) {
      ...
      initialTasks[task.section].push(task);
    }
  }
  if (!initialTasks.PRE_EVENT.length && !initialTasks.ON_DAY.length && !initialTasks.POST_EVENT.length) {
    initialTasks.PRE_EVENT.push({ title: "", relativeDays: 7, priority: "MEDIUM", section: "PRE_EVENT" });
  }

  const [tasksBySection, setTasksBySection] = useState(initialTasks);
  ...
  const addTask = (section: Section) => {
    ...
    setTasksBySection((prev) => ({
      ...prev,
      [section]: [...prev[section], { title: "", relativeDays: defaults[section], priority: "MEDIUM" as const, section }],
    }));
  };

  const removeTask = (section: Section, index: number) => {
    setTasksBySection((prev) => ({
      ...prev,
      [section]: prev[section].length <= 0 ? prev[section] : prev[section].filter((_, i) => i !== index),
    }));
  };

  const updateTask = (section: Section, index: number, field: keyof DefaultTask, value: string | number) => {
    setTasksBySection((prev) => ({
      ...prev,
      [section]: prev[section].map((t, i) => (i === index ? { ...t, [field]: value } : t)),
    }));
  };
          ...
          {SECTIONS.map((section) => (
            <div key={section}>
              <div className="flex items-center justify-between mb-2">
                ...
                <Button type="button" variant="secondary" size="sm" onClick={() => addTask(section)}>
                  <Plus className="h-3.5 w-3.5" />
                  Add
                </Button>
              </div>
              {tasksBySection[section].length === 0 ? (
                <p className="text-xs text-muted italic pl-1 pb-2">No tasks in this section</p>
              ) : (
                    ...
                    >
                      <div className="flex-1 space-y-2 min-w-0">
                        <input
                          type="text"
                          value={task.title}
                          onChange={(e) => updateTask(section, index, "title", e.target.value)}
                        ...
                        <div className="flex gap-2 items-center">
                          <div className="flex-1 min-w-0">
                            <input
                              ...
                              onChange={(e) =>
                                updateTask(section, index, "relativeDays", parseInt(e.target.value, 10) || 0)
                              }
                              placeholder="Days"
                              className={INPUT_CLASS}
                            />
                          ...
                          <div className="w-32 shrink-0">
                            <select
                              value={task.priority}
                              onChange={(e) =>
                                updateTask(section, index, "priority", e.target.value as DefaultTask["priority"])
                              }
                              className={INPUT_CLASS}
                            >
                              {PRIORITIES.map((p) => (
                      ...
                      <Button
                        ...
                        variant="ghost"
                        size="icon"
                        onClick={() => removeTask(section, index)}
    ...
    <div className="animate-fade-in">
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Create reusable checklists for your events"
          action={
            <Button size="md" onClick={openNewModal}>
              <Plus className="h-4 w-4" />
              New Template
            ...
            return (
              ...
              >
                ...
                {isExpanded && taskList.length > 0 && (
                  <div className="border-t border-border px-5 py-4 bg-background/30 space-y-4">
                    {SECTIONS.map((section) => {
